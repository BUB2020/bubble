...
池子|数量|占比|说明
---|:--:|:--|:--
流动性|200,000|1%|Uniswap流动性分批注入
BUB矿池|96,000,000|80%|用于BUB挖矿分配
邀请|2,000,000|10%|用于邀请奖励
奖池|1,800,000|9%|根据奖池余额分10轮释放，释放后的BUB会兑换成ETH充值进入奖池(具体请见下方表格）
...
* 矿主等级：质押1-5个ETH为LV1，质押6-10个ETH为LV2，质押大于11个ETH为LV3。


* 基础收益率：0.3%/天，年化109.5%。

* 质押上限：ETH质押数量无上限，但是最多只计算15个ETH的挖矿收益，超过的部分可以增加BUB挖矿上限。
...
* 挖矿上限：不是所有质押的BUB都能计算挖矿收益，能计算收益的部分由用户质押的ETH（取整）决定，当前比例为1比1000，即1个ETH能决定1000个BUB挖矿，超过部分不计算收益。
